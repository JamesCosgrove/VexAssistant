//
//  FetchTeams.swift
//  VexAssistant
//
//  Created by James Cosgrove on 25/03/2020.
//  Copyright © 2020 James Cosgrove. All rights reserved.
//

import Foundation

class FetchTeams: ObservableObject {
	
//	var willChange = PassthroughSubject<FetchEvents, Never>()
//
//	var eventList = Event(status: 0, size: 0, result: []) {
//		didSet {
//			willChange.send(self)
//		}
//	}
	@Published var teamData = Team(status: 0, size: 0, result: [])
	
	
	func fetchTeams(team: String) {
		var url = URLComponents(string: "https://api.vexdb.io/v1/get_teams")!

		url.queryItems = [URLQueryItem(name: "team", value: team)]
		
		let request = URLRequest(url: url.url!)
		
		URLSession.shared.dataTask(with: request) {(data, response, error) in
			guard let data = data else { return }
			
			let teamData = try! JSONDecoder().decode(Team.self, from: data)
			print(teamData)
			
			DispatchQueue.main.async {
				self.teamData = teamData
			}
		}.resume()
	}
	
	
}
